...
const AttractionDetail = () => {
  ...
  useEffect(() => {
    const fetchAttraction = async () => {
      try {
        const response = await fetch(`https://6729bdac6d5fa4901b6e27f4.mockapi.io/attractions/${id}`);
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const response = await fetch(`https://6729bdac6d5fa4901b6e27f4.mockapi.io/reviews?attractionId=${id}`);
  ...
  const handlePrevImageFullScreen = () => {
    if (attraction && attraction.images) {
      setCurrentImageIndex(
        (prevIndex) => (prevIndex - 1 + attraction.images.length) % attraction.images.length
      );
    }
  };
  ...
  return (
    <div className="attraction-detail">
      <button className="back-btn" onClick={() => navigate('/attractions')}>
        {t('backToList')}
      </button>
      ...
      <div className="modal__reviews">
          ...
          {reviews.length > 0 ? (
            reviews.map((review, index) => (
              <div key={index} className="review">
                ...
                <button className="review__delete" onClick={() => handleDeleteReview(review.id)}>
                  {t('deleteReview')}
                </button>
              </div>
            ))
        ...
        <form className="review-form" onSubmit={handleAddReview}>
          <input type="text" className="review-author" placeholder={t('yourName')} name="reviewAuthor" required />
          <textarea className="review-text" placeholder={t('yourReview')} name="reviewText" required />
          <div className="rating">
            {[1, 2, 3, 4, 5].map((rating) => (
              <span
                key={rating}
                className={`rating__star ${rating <= selectedRating ? 'selected' : ''}`}
                onClick={() => setSelectedRating(rating)}>★</span>
            ))}
          </div>
      ...
      {isFullScreen && (
        <div className="fullscreen-gallery">
          ...
          />
          <button className="gallery-prev-fullscreen" onClick={handlePrevImageFullScreen}>
            ←
          </button>
          <button className="gallery-next-fullscreen" onClick={handleNextImageFullScreen}>
            →
          </button>
        </div>
      )}
