...
const GuaranteesSlider = () => {
    ...
    const prevSlide = () => {
        setCurrentSlide(
            (prev) => (prev - 1 + guarantees.length) % guarantees.length,
        );
    };
        ...
        <section className="slider">
            ...
            <div className="wrapper">
                <i id="left" onClick={prevSlide}>
                    <img
                        src={`${process.env.PUBLIC_URL}/assets/img/left.svg`}
                        alt="left"
                        width="50px"
                        height="20px"
                    />
                ...
                <ul className="carousel">
                    {guarantees.map((guarantee, index) => (
                        <li
                            className={`card ${index === currentSlide ? "active" : "hidden"}`}
                            key={index}
                        >
                            <div className="slider__box">
                                <img
                                    src={`${process.env.PUBLIC_URL}${guarantee.icon}`}
                                    alt={guarantee.title}
                                    width="40px"
                                    height="40px"
                                />
                ...
                <i id="right" onClick={nextSlide}>
                    <img
                        src={`${process.env.PUBLIC_URL}/assets/img/right.svg`}
                        alt="right"
                        width="50px"
                        height="20px"
                    />
